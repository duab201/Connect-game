...
class Board:
    ...
    def __init__(self,rows=6,cols=7,streak=4):
        ...
        if rows > 0:
            ...
            raise ValueError('Ivalid height')
        if cols > 0:
            self._width = cols
        else:
            raise ValueError('Ivalid width')
        if streak > 0 and streak <= min(rows, cols):
            self._streak = streak
        else:
            raise ValueError('Ivalid streak')
    ...
    def getColor(self,r,c):
        ...
        board_height = self.getHeight()
        board_width = self.getWidth()
        if (isinstance(r, int) and r >= 0 and r < board_height):
            if (isinstance(c, int) and c >= 0 and c < board_width):
    ...
    def getLastMove(self):
        ...
        """
        my_last = ()
        if self._moves == my_last:
            return None
        else:
            my_last = self._moves[-1]
            return my_last
    ...
    def findAvailableRow(self,col):
        ...
        """
        my_column = self.getWidth()
        my_row = self.getHeight()
        lowest_row = self.getHeight()
        if isinstance(col, int) and col >= 0 and col < (my_column):
    ...
    def place(self,column,color):
        ...
        if (isinstance(column, int) and column >= 0 and column < my_column):
            if self.isFullColumn(column):
                return -1
            else:
                for i in range(my_row):
                    if self._board[i][column] == '' :
                        self._board[i][column] = color
                        row = i
    ...
    def undoPlace(self):
        ...
        """
        if self._moves != '':
            my_row , my_column = self._moves[-1]
            self._board[my_row][my_column] = ''
            self._moves.pop()
...
#### TASK 4 ####
    ...
    def findAcross(self,r,c,leng):
        ...
        while c2 < my_col and self._board[r][c2] == color:
            c2 = c2 + 1
            
        if dist(self, r ,c1 + 1, r ,c2 - 1) >= leng:
            return (r, c1+ 1, r ,c2- 1)
        else:
            return None
    ...
    def findSWNE(self,r,c,leng):
        ...
        #    color does not match the color of the original piece
        # 3. The position should end where the colors match, so you need to "back up"
        # 4. Use dist to make sure the length of the run is long enough assert type(r) == int
        assert type(r) == int
        assert type(c) == int
        assert type(leng) == int and leng > 0
        assert in_range(self,r,c)
        ...
        while (c1  - diagonal_count) >= 0 and (r1 + diagonal_count) < my_row and self._board[r1 + diagonal_count][c1 - diagonal_count] == color:
            c1 = (c1 - diagonal_count)
            r1 = (r1 + diagonal_count)
            
        while (c2 + diagonal_count < my_col) and (r2 - diagonal_count >= 0) and self._board[r2 - diagonal_count][c2 + diagonal_count] == color:
             c2 = (c2 + diagonal_count)
             r2 = (r2 - diagonal_count)
    ...
    def findNWSE(self,r,c,leng):
        ...
        if leng == 0:
            ...
            while c1 >= 0 and r1 >= 0 and self._board[r1][c1] == color:
                c1 = (c1 - diagonal_count)
                r1 = (r1 - diagonal_count)
            
            while (c2 < my_col) and (r2 < my_row) and self._board[r2][c2] == color:
                 c2 = (c2 + diagonal_count)
                 r2 = (r2 + diagonal_count)
            
            if dist(self, (r1 + diagonal_count) ,(c1 + diagonal_count), (r2 - diagonal_count) , (c2 - diagonal_count)) >= leng:
                return ( (r1 + diagonal_count) ,(c1 + diagonal_count), (r2 - diagonal_count) , (c2 - diagonal_count) )
            else:
                return None
